...
from src.ddp_train import train, inference
...
if __name__ == "__main__":
    ...
    if opt.mode.lower() == 'train':
        ...
        for epoch in range(startEpoch+1, opt.nEpochs+1):
            epoch_train_loss = train(trainSet, ddp_model, 
                                    criterion, optimizer, 
                                    device, opt.batch_size, epoch)
            epoch_val_loss, val_metrics = inference(valSet, ddp_model, criterion, 
                                                    evaluator, metrics,
        ...
        test_loss, test_metrics = inference(testSet, ddp_model, criterion, 
                                            evaluator, metrics,
                                            device, opt.batch_size)
