...
def train(
    ...
    train_progress_bar = tqdm(dataloader, desc=f"Epoch {epoch} [Training]", position=0, leave=True)
    for batch_id, data in enumerate(train_progress_bar,start_iter):
    # for batch_id, data in enumerate(dataloader, start_iter):
        
        optimizer.zero_grad()
        input_ids = data['input_ids'].to(accelerator.device)
        attention_masks = data['attention_mask'].to(accelerator.device)
        targets = data['labels'].to(accelerator.device)

        embeddings = model(input_ids, attention_masks)
        loss = criterion(embeddings, targets).to(accelerator.device)
